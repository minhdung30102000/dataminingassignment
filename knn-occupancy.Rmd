...
propDiag2 <- round(prop.table(cntDiag) * 100 , digits = 1)
cntDiag2
propDiag2
```
For the trainning model, I will chane the value in "occupancy" in factor with "0" for ‘1’ for ‘occupied’ and ‘0’ for ‘not occupied.’
```{r}
...
summary(cleanDF_n3)
```
Now I will divide the data set into 2 test and training set with the proportion is 60% for train and 40% for test. I also We’ll create separate training and tests sets for the labels
```{r}
